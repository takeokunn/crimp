...
class GymReviewsController < ApplicationController
  ...
  def calculate_gym_ranking
    ...
    @overall_score /= @gymReviews.length
    @problems_quality /= @gymReviews.length
    @cost_par_fee /= @gymReviews.length
    @comfortableness /= @gymReviews.length
    @service /= @gymReviews.length
